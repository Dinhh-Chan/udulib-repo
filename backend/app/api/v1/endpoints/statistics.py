...
async def get_overview_statistics(
    ...
    # Tổng số môn học
    total_subjects = await db.scalar(text("SELECT COUNT(*) FROM subjects"))
    
    # Tổng số ngành học
    total_majors = await db.scalar(text("SELECT COUNT(*) FROM majors"))
    
    # Tổng lượt xem
    total_views = await db.scalar(text("SELECT COALESCE(SUM(view_count), 0) FROM documents"))
    
    # Tổng lượt tải
    total_downloads = await db.scalar(text("SELECT COALESCE(SUM(download_count), 0) FROM documents"))
    
    # Tổng số bình luận
    total_comments = await db.scalar(text("SELECT COUNT(*) FROM comments"))
...
async def get_storage_usage(
    ...
    total_size_result = await db.execute(text("SELECT SUM(file_size) as total FROM documents"))
...
async def get_forum_activity(
    ...
    """
    # Tổng số posts
    total_posts = await db.scalar(text("SELECT COUNT(*) FROM forum_posts"))
    
    # Tổng số replies
    total_replies = await db.scalar(text("SELECT COUNT(*) FROM forum_replies"))
    
    # Forum có nhiều posts nhất
    most_active_forums = await db.execute(text("""
